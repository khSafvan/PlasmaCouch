...
echo "=== PlasmaCouch Verification & Test Suite ==="

# Step 1: Shell Scripts Syntax Verification
echo "--> [1/3] Checking Shell Script Syntax (bash -n)..."
mapfile -t SHELL_SCRIPTS < <(find "${REPO_ROOT}" -type f \( -name "Messages.sh" -o -name "*.sh" -o -path "*/bin/plasmacouch*" \) -not -path "*/.git/*" -not -name "*.desktop*")
...
for script in "${REPO_ROOT}/bin/plasmacouch-common-env" "${REPO_ROOT}/bin/plasmacouch-swap-session" "${REPO_ROOT}/scripts/verify-all.sh"; do
    if [ -f "${script}" ]; then
        chmod +x "${script}"
